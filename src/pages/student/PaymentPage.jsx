...
export function PaymentPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <p className="text-taxo-light">Choose your plan, complete payment using the official manual method shared by admin, then upload the receipt image or PDF. Your booking is confirmed only after admin review.</p>
        <p className="rtl mt-3 text-taxo-light">اختر خطتك، ثم أتم الدفع بالطريقة الرسمية التي تحددها الإدارة، وبعدها ارفع صورة أو ملف إثبات الدفع. لن يتم تأكيد الحجز إلا بعد مراجعة الإدارة.</p>
      </Card>
      <div className="grid gap-4 md:grid-cols-3">
        {allPlans.map((plan) => (
          <Card key={plan.id}>
            <Wallet className="mb-3 text-taxo-gold" />
            <h3 className="font-bold">{plan.label}</h3>
            <p className="mt-2 text-2xl font-black">{plan.amount} EGP</p>
            <p className="mt-2 text-xs text-taxo-light/70">Manual confirmation after proof upload.</p>
          </Card>
        ))}
      </div>
