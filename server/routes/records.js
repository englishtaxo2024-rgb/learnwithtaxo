...
function scoped(db, user) {
  ...
  if (user.role === 'teacher') {
    ...
    const students = db.students.filter((student) => student.teacherId === teacherId || studentIds.has(student.id));
    ...
    return {
      ...
      groups,
      students,
      materials: db.materials.filter((material) => groups.some((group) => group.materialKeys?.includes(material.key))),
      payments: db.payments.filter((payment) => scopedStudentIds.has(payment.student_id)),
      placementSubmissions: db.placementSubmissions.filter((submission) => submission.assigned_teacher_id === teacherId || submission.assigned_teacher === teacher?.name)
    };
  }
  const student = db.students.find((item) => item.id === user.studentId || item.email === user.email || item.parentEmail === user.email || item.phone === user.phone);
  const group = db.groups.find((item) => item.id === student?.groupId);
  return {
    ...db,
    teachers: db.teachers.filter((teacher) => teacher.id === student?.teacherId),
    groups: group ? [group] : [],
    students: student ? [student] : [],
    materials: db.materials.filter((material) => student?.materialKeys?.includes(material.key) || group?.materialKeys?.includes(material.key)),
    payments: student ? db.payments.filter((payment) => payment.student_id === student.id) : [],
    placementSubmissions: student ? db.placementSubmissions.filter((submission) => submission.student_id === student.id || submission.student_phone === student.phone || submission.parent_phone === student.phone) : [],
    audit: []
  };
}
...
function questionList(settings, testType) {
  return settings.testTypes.find((item) => item.id === testType || item.name === testType)?.questions || [];
}

function scoreAnswers(settings, testType, answers = []) {
  const questions = questionList(settings, testType);
  ...
  const earned = answers.reduce((sum, answer) => {
    ...
    if (!question || question.type !== 'mcq' || !question.answer) return sum;
    return sum + (String(answer.answer || '').trim() === String(question.answer).trim() ? Number(question.points || 0) : 0);
  }, 0);
  const percent = max ? Math.round((earned / max) * 100) : null;
  const level = percent == null ? 'Under Review' : settings.levelRules.find((rule) => percent >= rule.min && percent <= rule.max)?.level || 'Under Review';
  return { score: percent, estimated_level: level };
}

router.get('/placement/settings-public', async (_req, res) => {
  const db = await loadDb();
  const safeSettings = {
    ...db.placementSettings,
    testTypes: db.placementSettings.testTypes.map((type) => ({
      ...type,
      questions: type.questions.map(({ answer, ...question }) => question)
    }))
  };
  res.json({ settings: safeSettings });
});

router.post('/placement/submissions', (_req, res) => {
  res.status(410).json({
    error: 'This legacy placement endpoint is disabled. Verify a one-time placement code before submitting.'
  });
});
...
router.get('/bootstrap', async (req, res) => {
  ...
  res.json({
    ...
    placementTests: db.placementTests,
    placementSettings: req.user.role === 'admin' ? db.placementSettings : undefined,
    placementSubmissions: db.placementSubmissions,
    finalTests: db.finalTests,
  ...
  const result = await mutateDb((db) => {
    if (req.params.kind === 'students') {
      rows.forEach((row) => {
        ...
        const student = studentFromTaxoRow(normalized);
      ...
      rows.forEach((row) => {
        ...
        const group = groupFromTaxoRow(normalized, []);
...
router.post('/sync/google-students', requireRole('admin'), async (req, res) => {
  ...
  const imported = await mutateDb((db) => {
    sourceRows.forEach((row) => {
      ...
      if (!id) return;
      const teacher = db.teachers.find((item) => item.name === (row.Teacher || row.teacher));
      const student = {
        id,
        name: row['Student Name'] || row.name,
        parentEmail: row['Parent Email'] || row.parentEmail,
        phone: row.Phone || row.phone,
        course: row.Course || row.Material || row.course,
        level: row.Level || row.level,
        teacherId: teacher?.id,
        teacher: teacher?.name || row.Teacher || row.teacher,
        groupName: row['Group Name'] || row.groupName,
        currentSession: row['Current Session'] || row.currentSession,
        paymentStatus: row.Payment || row.paymentStatus || 'Imported',
...
router.post('/payments', upload.single('proof'), async (req, res) => {
  const payment = await mutateDb((db) => {
    const student = db.students.find((item) => item.id === req.body.student_id || item.id === req.user.studentId);
    if (!student) throw new Error('Student not found');
    if (req.user.role === 'student' && req.user.studentId !== student.id) throw new Error('Forbidden');
    if (req.user.role === 'teacher' && student.teacherId !== req.user.id) throw new Error('Forbidden');
    const group = db.groups.find((item) => item.id === student.groupId);
    const record = {
      ...
      payment_method: req.body.payment_method || 'Manual transfer',
      screenshot_url: fileUrl(req.file),
      status: 'Pending',
    ...
    audit(db, req.user, 'payment.submit', { paymentId: record.payment_id, studentId: student.id });
    return record;
  });
  res.json({ payment });
});
...
router.patch('/payments/:id', requireRole('admin'), async (req, res) => {
  const payment = await mutateDb((db) => {
    const record = db.payments.find((item) => item.payment_id === req.params.id);
    if (!record) throw new Error('Payment not found');
    record.status = req.body.status || record.status;
    record.admin_note = req.body.admin_note ?? record.admin_note;
    record.reviewed_at = new Date().toISOString();
    record.reviewed_by = req.user.email;
    const student = db.students.find((item) => item.id === record.student_id);
    if (student) student.paymentStatus = record.status === 'Paid' ? 'Paid' : record.status;
    audit(db, req.user, 'payment.review', { paymentId: record.payment_id, status: record.status });
    return record;
  });
  res.json({ payment });
});
...
router.patch('/placement/submissions/:id', async (req, res) => {
  const submission = await mutateDb((db) => {
    const record = db.placementSubmissions.find((item) => item.placement_test_id === req.params.id);
    if (!record) throw new Error('Placement submission not found');
    if (req.user.role === 'teacher' && record.assigned_teacher_id !== req.user.id) throw new Error('Forbidden');
    if (req.user.role === 'student' && record.student_id !== req.user.studentId) throw new Error('Forbidden');
    const allowed = req.user.role === 'admin'
      ? ['score', 'estimated_level', 'assigned_teacher', 'assigned_teacher_id', 'assigned_group', 'assigned_group_id', 'status', 'admin_notes']
      : ['teacher_feedback', 'estimated_level', 'status'];
    allowed.forEach((key) => {
      if (key in req.body) record[key] = req.body[key];
    });
    record.reviewed_at = new Date().toISOString();
    audit(db, req.user, 'placement.review', { id: record.placement_test_id, status: record.status });
    return record;
  });
  res.json({ submission });
});
...
router.patch('/placement/settings', requireRole('admin'), async (req, res) => {
  const settings = await mutateDb((db) => {
    db.placementSettings = { ...db.placementSettings, ...req.body };
    audit(db, req.user, 'placement.settings.update');
    return db.placementSettings;
  });
  res.json({ settings });
});
...
router.post('/materials', requireRole('admin'), upload.array('slides', 80), async (req, res) => {
  ...
  const material = await mutateDb((db) => {
    const payload = {
      ...
      level,
      session,
      slides: (req.files || []).map((file, index) => ({ index: index + 1, originalName: file.originalname, url: `/uploads/${file.filename}` })),
    ...
    if (assignToGroupId) {
      const group = db.groups.find((item) => item.id === assignToGroupId);
      if (group) group.materialKeys = [...new Set([...(group.materialKeys || []), key])];
    }
    if (assignToStudentId) {
      const student = db.students.find((item) => item.id === assignToStudentId);
      if (student) student.materialKeys = [...new Set([...(student.materialKeys || []), key])];
    }
